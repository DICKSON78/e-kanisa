...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between mb-2">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Picha na Video</h1>
                <p class="text-sm text-gray-500">Usimamizi wa picha na video za kanisa</p>
            </div>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('gallery.create') }}" class="rx-btn rx-btn-primary flex items-center gap-2">
                <i class="fas fa-plus"></i>
                <span class="hidden sm:inline">Ongeza Galeri</span>
            </a>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="rx-stat-card rounded-2xl">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-xs font-medium text-gray-500 mb-1">Jumla ya Galeri</p>
                    <p class="text-2xl font-bold text-gray-900">{{ $stats['total_galleries'] ?? 0 }}</p>
                </div>
                <div class="w-12 h-12 rounded-xl flex items-center justify-center" style="background: rgba(54,9,88,0.08)">
                    <i class="fas fa-images" style="color: #360958"></i>
                </div>
            </div>
        </div>

        <!-- Jumla ya Picha/Video -->
        <div class="rx-stat-card rounded-2xl">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-xs font-medium text-gray-500 mb-1">Jumla ya Picha/Video</p>
                    <p class="text-2xl font-bold" style="color: #3b82f6">{{ $stats['total_items'] ?? 0 }}</p>
                </div>
        ...
        <div class="rx-stat-card rounded-2xl">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-xs font-medium text-gray-500 mb-1">Ukubwa Jumla</p>
                    <p class="text-2xl font-bold" style="color: #16a34a">{{ $stats['total_size'] ?? '0 MB' }}</p>
                </div>
                <div class="w-12 h-12 rounded-xl flex items-center justify-center" style="background: rgba(22,163,74,0.1)">
                    <i class="fas fa-hdd" style="color: #16a34a"></i>
                </div>
            </div>
        </div>
        ...
        <div class="rx-stat-card rounded-2xl">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold" style="color: #7c3aed">{{ $stats['this_month'] ?? 0 }}</p>
                </div>
    ...
    <div class="rx-card rounded-xl p-4 mb-2">
        <form id="filterForm" class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <div class="rx-search">
                        ...
                        <input type="text" name="search" id="searchInput" value="{{ request('search') }}" placeholder="Tafuta kwa kichwa...">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <select name="status" id="statusFilter" class="rx-select">
                        ...
                        <option value="published" {{ request('status') === 'published' ? 'selected' : '' }}>Imechapishwa</option>
                    </select>
                </div>
            ...
            <div class="flex justify-end pt-1">
                <button type="button" onclick="clearFilters()" class="rx-btn rx-btn-secondary flex items-center justify-center gap-2 text-sm">
    ...
    <!-- Gallery Grid -->
    <div class="rx-card rounded-2xl overflow-hidden" id="galleryGridContainer">
        @include('panel.gallery._grid')
    </div>
</div>
...
// ========================================
function confirmAction(action, galleryId, galleryTitle) {
    ...
    if (action === 'delete') {
        ...
        message.textContent = 'Je, una uhakika unataka kufuta galeri hii? Hatua hii haiwezi kutenduliwa.';
        nameEl.textContent = galleryTitle;
...
function executeAction() {
    if (!currentAction || !currentGalleryId) return;
    ...
    const formId = `${currentAction}-form-${currentGalleryId}`;
    ...
    if (form) {
        ...
        .then((data) => {
            ...
            setTimeout(() => filterGalleries(), 500);
        })
        .catch((err) => {
...
// ========================================
function filterGalleries(page = 1) {
    ...
    searchTimeout = setTimeout(() => {
        ...
        const newUrl = '{{ route("gallery.index") }}' + (params.toString() ? '?' + params.toString() : '');
...
function attachPaginationListeners() {
    document.querySelectorAll('#galleryGridContainer .pagination a').forEach(link => {
        link.addEventListener('click', function(e) {
            e.preventDefault();
            const url = new URL(this.href);
            const page = url.searchParams.get('page') || 1;
            filterGalleries(parseInt(page));
        });
    });
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('statusFilter')?.addEventListener('change', () => filterGalleries());
    document.getElementById('yearFilter')?.addEventListener('change', () => filterGalleries());
...
window.addEventListener('popstate', function() {
    ...
    filterGalleries(parseInt(urlParams.get('page')) || 1);
});
