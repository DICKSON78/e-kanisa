...
@section('title', 'Malipo ya Mtandaoni - Mfumo wa E-Kanisa')
@section('page-title', 'Malipo ya Mtandaoni')
@section('page-subtitle', 'Rekodi na usimamizi wa malipo yaliyofanywa kupitia mitandao')
...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between mb-2">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Malipo ya Mtandaoni</h1>
                <p class="text-sm text-gray-500">Rekodi na usimamizi wa malipo yaliyofanywa kupitia mitandao</p>
            </div>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('online-giving.create') }}" class="rx-btn rx-btn-primary flex items-center gap-2">
                <i class="fas fa-plus"></i>
                <span class="hidden sm:inline">Ongeza Malipo</span>
            </a>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="rx-stat-card rounded-2xl">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-xs font-medium text-gray-500 mb-1">Jumla ya Mitandao</p>
        ...
        <div class="rx-stat-card rounded-2xl">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-xs font-medium text-gray-500 mb-1">Jumla ya Kiasi</p>
                    <p class="text-2xl font-bold" style="color: #16a34a">{{ number_format($stats['completed_amount'] ?? 0, 0) }} TSh</p>
                </div>
        ...
        <div class="rx-stat-card rounded-2xl">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold" style="color: #ca8a04">{{ number_format($stats['pending_count'] ?? 0) }}</p>
                </div>
        ...
        <div class="rx-stat-card rounded-2xl">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-xs font-medium text-gray-500 mb-1">Kiasi cha Leo</p>
                    <p class="text-2xl font-bold" style="color: #3b82f6">{{ number_format($stats['today_amount'] ?? 0, 0) }} TSh</p>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div>
                    ...
                    <select name="status" id="statusFilter" class="rx-select">
                        ...
                        <option value="Imeshindwa" {{ request('status') == 'Imeshindwa' ? 'selected' : '' }}>Imeshindwa</option>
                        <option value="Imerejeshwa" {{ request('status') == 'Imerejeshwa' ? 'selected' : '' }}>Imerejeshwa</option>
                    </select>
                </div>

                <!-- Payment Method -->
                <div>
                    <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">Njia ya Malipo</label>
                    <select name="payment_method" id="paymentMethodFilter" class="rx-select">
                        <option value="">Zote</option>
                        <option value="M-Pesa" {{ request('payment_method') == 'M-Pesa' ? 'selected' : '' }}>M-Pesa</option>
                        <option value="Tigo Pesa" {{ request('payment_method') == 'Tigo Pesa' ? 'selected' : '' }}>Tigo Pesa</option>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <div class="rx-search">
                        ...
                        <input type="text" name="search" id="searchInput" value="{{ request('search') }}" placeholder="Tafuta kwa jina, namba, rejea...">
                    </div>
                </div>
    ...
    <!-- Transactions Table -->
    <div class="rx-card rounded-2xl overflow-hidden" id="onlineGivingTableContainer">
        @include('panel.online-giving._table')
    </div>
...
// ========================================
function confirmAction(action, transactionId, label) {
    ...
    if (action === 'approve') {
        ...
        title.textContent = 'Thibitisha Malipo';
        message.textContent = 'Je, una uhakika unataka kuthibitisha malipo haya?';
        ...
        title.textContent = 'Kataa Malipo';
        message.textContent = 'Je, una uhakika unataka kukataa malipo haya?';
        nameEl.textContent = label;
...
function executeAction() {
    if (!currentAction || !currentTransactionId) return;
    ...
    const formId = `${currentAction}-form-${currentTransactionId}`;
    ...
    if (form) {
        ...
        .then((data) => {
            showNotification(data.message || 'Imefanikiwa', 'success', 'Imefanikiwa');
            closeConfirmActionModal();
            setTimeout(() => filterTransactions(), 500);
        })
        .catch((err) => {
            const msg = err?.message || 'Hitilafu imetokea. Tafadhali jaribu tena.';
            showNotification(msg, 'error', 'Hitilafu');
        })
        .finally(() => {
            btn.disabled = false;
            btnIcon.className = 'fas fa-check';
        ...
        showNotification('Fomu haijapatikana. Tafadhali jaribu tena.', 'error', 'Hitilafu');
        closeConfirmActionModal();
    }
}
...
function filterTransactions(page = 1) {
    ...
    searchTimeout = setTimeout(() => {
        ...
        const container = document.getElementById('onlineGivingTableContainer');
        if (container) {
            container.style.opacity = '0.5';
            container.style.pointerEvents = 'none';
        }

        const newUrl = '{{ route("online-giving.index") }}' + (params.toString() ? '?' + params.toString() : '');
...
function attachPaginationListeners() {
    document.querySelectorAll('#onlineGivingTableContainer .pagination a').forEach(link => {
        link.addEventListener('click', function(e) {
            e.preventDefault();
            const url = new URL(this.href);
            const page = url.searchParams.get('page') || 1;
            filterTransactions(parseInt(page));
        });
    });
}
...
window.addEventListener('popstate', function() {
    ...
    document.getElementById('paymentMethodFilter').value = urlParams.get('payment_method') || '';
    ...
    filterTransactions(parseInt(urlParams.get('page')) || 1);
});
